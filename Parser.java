...
public class Parser {
   ...
   public Node parseDef() {
     ...
     if ( token.isKind("rparen") ) {
       ...
       return new Node("def", expr, null, null)
     } else {
        ...
        return new Node("def", params, expr, null);
     }
   ...
   public Node parseParams() {
     ...
     if ( token.isKind("name") ) {
        lex.putBackToken(token);
        Node params = parseParams();
        return new Node("params", params, null, null);
     }
     else {
   ...
   public Node parseExpr() {
     ...
     if ( token.isKind("number") ) {
        ...
        lex.putBackToken(token);
        Node list = parseList();
        return new Node("expr", list, null, null);
     }
   }
   ...
   public Node parseList() {
     ...
     token = lex.getNextToken();
     if( token.isKind("rparen") ) {
        return Node("list", null, null, null);
     }
     else {
        lex.putBackToken(token);
